...
spawn_bg "fast-drift" cargo run -q --release -p acmex-gen-hooks -- --target pre-commit --check
spawn_bg "manifest-drift" cargo run -q --release -p acmex-manifest-audit -- --check
