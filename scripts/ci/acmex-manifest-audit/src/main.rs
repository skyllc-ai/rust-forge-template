...
/// symmetry with `acmex-gen-hooks` / `acmex-gen-workflow` so the pre-push hook
/// output reads consistently across drift detectors.
#[derive(Debug, Parser)]
#[command(
    name = "acmex-manifest-audit",
...
struct Args {
    ...
    /// symmetry with `acmex-gen-hooks` / `acmex-gen-workflow`.
...
fn main() -> ExitCode {
    acmex_version::handle_version!("acmex-manifest-audit");
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(err) => {
            eprintln!("❌ acmex-manifest-audit: {err:#}");
            ExitCode::from(1)
        }
    }
}
...
fn run() -> Result<()> {
    ...
    if findings.is_empty() {
        if args.verbose {
            eprintln!(
                "✅ acmex-manifest-audit: workspace manifests pass all 15 Phase-1 invariants \
                 ({} member(s) checked)",
        ...
        anyhow::bail!(
            "acmex-manifest-audit detected {} drift finding(s)",
            findings.len()
        );
    }
}
...
fn emit_findings_report(findings: &[Finding]) {
    eprintln!(
        "❌ acmex-manifest-audit: {} drift finding(s) detected:",
        findings.len()
    );
    eprintln!();
