...
pub(crate) struct Finding {
    ...
    /// Member crate id (e.g. `"acmex-core"`) for member-scoped findings,
...
struct KnownExceptions {
    ...
    /// hyphen convention).  Source: acmex-gen-workflow comment in
    /// `scripts/ci/acmex-gen-workflow/src/main.rs`.
    ...
    /// tailored to their specific domain (e.g. `acmex-time` is a
...
impl KnownExceptions {
    ...
    const fn new() -> Self {
        ...
        // by default.
        Self {
            publish_explicit: BTreeSet::new(),
            underscore_bin_ok: BTreeSet::new(),
            nonworkspace_dep_ok: BTreeSet::new(),
            readme_override_ok: BTreeSet::new(),
            keywords_override_ok: BTreeSet::new(),
            categories_override_ok: BTreeSet::new(),
        }
    }
}
...
pub(crate) struct DiscoveredMember<'a> {
    /// Workspace-relative path (e.g. `"crates/acmex-core/Cargo.toml"`).
...
/// `"crates/acmex-core/Cargo.toml"` → `"acmex-core"`.
...
fn audit_root_is_virtual(root: &RootManifest) -> Vec<Finding> {
    if root.package.is_some() {
        vec![Finding {
            invariant: "3.9",
            member: "<root>".to_owned(),
            detail: "root Cargo.toml has a [package] block; ACMEX uses a virtual workspace"
                .to_owned(),
        }]
    } else {
        Vec::new()
...
fn audit_resolver_pinned(root: &RootManifest) -> Vec<Finding> {
    ...
    match ws.resolver.as_deref() {
        ...
        Some(other) => vec![Finding {
            invariant: "3.10",
            member: "<root>".to_owned(),
            detail: format!("[workspace].resolver = {other:?}; ACMEX requires \"3\""),
        }],
        None => vec![Finding {
            invariant: "3.10",
            member: "<root>".to_owned(),
            detail: "[workspace].resolver is unset; ACMEX requires \"3\"".to_owned(),
        }],
    }
}
...
/// the documented `acmex-diag` exception).
...
mod tests {
    ...
    /// Test-local exception set that lists the fictional crate
    /// `acmex-listed` in every escape hatch, so the suppression
    /// mechanism stays covered even though the production allow-lists
    /// in [`KnownExceptions::new`] start empty in a fresh project.
    fn listed_exceptions() -> KnownExceptions {
        let listed: BTreeSet<&'static str> = core::iter::once("acmex-listed").collect();
        KnownExceptions {
            publish_explicit: listed.clone(),
            underscore_bin_ok: listed.clone(),
            nonworkspace_dep_ok: core::iter::once(("acmex-listed", "special-sys")).collect(),
            readme_override_ok: listed.clone(),
            keywords_override_ok: listed.clone(),
            categories_override_ok: listed,
        }
    }
...
"#;
    ...
    fn missing_lints_block_fires_3_7() {
        ...
        let d = disc("crates/acmex-core/Cargo.toml", &m);
        let findings = audit_lints_inherit_workspace(&d, "acmex-core");
    ...
    fn profile_block_fires_3_8() {
        ...
        let m = parse_member(&text).unwrap();
        let d = disc("crates/acmex-core/Cargo.toml", &m);
        let findings = audit_no_workspace_policy_blocks(&d, "acmex-core");
        assert_eq!(findings.len(), 1);
    ...
    fn non_workspace_dep_fires_3_6() {
        ...
        let d = disc("crates/acmex-core/Cargo.toml", &m);
        let exc = KnownExceptions::new();
        let findings = audit_deps_inherit_workspace(&d, "acmex-core", &exc);
        assert_eq!(findings.len(), 1);
    ...
    fn known_exception_suppresses_3_6() {
        let text = MEMBER_CLEAN.replace(
            "anyhow = { workspace = true }",
            r#"special-sys = { version = "0.1.44", features = ["extended"] }"#,
        );
        let m = parse_member(&text).unwrap();
        let d = disc("crates/acmex-listed/Cargo.toml", &m);
        let exc = listed_exceptions();
        let findings = audit_deps_inherit_workspace(&d, "acmex-listed", &exc);
        assert!(
            findings.is_empty(),
    ...
    fn missing_docs_rs_metadata_fires_3_15() {
        ...
        let m = parse_member(&text).unwrap();
        let d = disc("crates/acmex-core/Cargo.toml", &m);
        let findings = audit_docs_rs_metadata_present(&d, "acmex-core");
        assert_eq!(findings.len(), 1);
    ...
    fn underscore_bin_name_fires_3_13_except_diag() {
        ...
        let d = disc("crates/acmex-core/Cargo.toml", &m);
        let exc = KnownExceptions::new();
        let findings = audit_lib_bin_name_convention(&d, "acmex-core", &exc);
        assert_eq!(findings.len(), 1);
    ...
    fn normalise_path_dep_resolves_relative() {
        assert_eq!(
            normalise_path_dep("crates/acmex-cli/Cargo.toml", "../acmex-client"),
            "crates/acmex-client"
        );
        assert_eq!(
            normalise_path_dep(
                "scripts/ci/acmex-gen-hooks/Cargo.toml",
                "../../../crates/acmex-core"
            ),
            "crates/acmex-core"
        );
    ...
    fn readme_override_on_unlisted_crate_still_fires_3_5() {
        ...
        let d = disc("crates/acmex-core/Cargo.toml", &m);
        let exc = KnownExceptions::new();
        let findings = audit_metadata_fields(&d, "acmex-core", &exc);
    ...
    fn readme_override_to_non_readme_path_still_fires_even_for_listed_crate() {
        // `acmex-listed` is on the allow-list, but the exception is
    ...
    fn readme_workspace_inherit_passes_for_every_member() {
        ...
        let m = parse_member(MEMBER_CLEAN).unwrap();
        let exc = KnownExceptions::new();
        for id in ["acmex-core", "acmex-time", "acmex-text", "acmex-broker"] {
    ...
    fn member_id_from_path_strips_prefix_and_filename() {
        assert_eq!(
            member_id_from_path("crates/acmex-core/Cargo.toml"),
            "acmex-core"
        );
        assert_eq!(
            member_id_from_path("scripts/ci/acmex-gen-hooks/Cargo.toml"),
            "acmex-gen-hooks"
        );
    }
    ...
    fn keywords_override_on_allowlisted_crate_is_suppressed() {
        // A crate on the keywords allow-list using a well-formed
        // non-empty array must not fire.
        let text = MEMBER_CLEAN.replace(
            "keywords.workspace = true",
    ...
    fn empty_keywords_array_on_allowlisted_crate_still_fires_3_5() {
        // `acmex-listed` is on the allow-list but the override is
