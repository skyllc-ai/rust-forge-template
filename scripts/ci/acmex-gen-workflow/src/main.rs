...
struct Args {
    ...
    /// with `acmex-gen-hooks`).  Exits 1 if drift is detected.
...
fn main() -> ExitCode {
    acmex_version::handle_version!("acmex-gen-workflow");
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(err) => {
            eprintln!("❌ acmex-gen-workflow: {err:#}");
            ExitCode::from(1)
        }
    }
}
...
fn run() -> Result<()> {
    ...
    if report.is_empty() {
        ...
        eprintln!(
            "❌ acmex-gen-workflow: {} structural drift issue(s) detected:",
            report.len()
        );
        for issue in &report {
            eprintln!("   • {issue}");
