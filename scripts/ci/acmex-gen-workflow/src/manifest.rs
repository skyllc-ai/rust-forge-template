...
//!
//! # Why duplicate (a subset of) `scripts/ci/acmex-gen-hooks/src/manifest.rs`?
//!
//! `acmex-gen-hooks` and `acmex-gen-workflow` both read
//! `scripts/ci/gates.toml`. A purist refactor would extract a shared
//! `acmex-gates-schema` library crate that both binaries depend on.  We
//! deliberately deferred that for this PR for two reasons:
//!
//! 1. The cross-tool dependency graph is currently zero-coupled
//!    (acmex-gen-hooks doesn't depend on acmex-gen-workflow and vice versa); a
//!    shared crate would introduce a new internal dependency just to hold ~30
...
//! shared validator), promote this module + the acmex-gen-hooks one into a
...
"#;
    ...
    fn unknown_fields_are_silently_ignored() {
        // Documents the schema-coupling design decision: acmex-gen-hooks
        // can add new manifest fields without breaking acmex-gen-workflow.
        let with_extra = r#"
[[gate]]
id = "fmt"
label = "fmt"
