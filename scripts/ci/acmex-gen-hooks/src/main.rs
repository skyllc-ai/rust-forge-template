...
//!
//! ```text
//! acmex-gen-hooks [--check] [--target {pre-push|pre-commit}] [--verbose]
//! ```
//!
//! # Exit codes
//!
...
fn main() -> ExitCode {
    acmex_version::handle_version!("acmex-gen-hooks");
    let args = Args::parse();
    match run(&args) {
        Ok(code) => code,
        Err(err) => {
            eprintln!("acmex-gen-hooks: {err:#}");
            ExitCode::from(2)
        }
    }
}
...
fn run(args: &Args) -> Result<ExitCode> {
    ...
    if args.check {
        ...
        if on_disk == emitted {
            if args.verbose {
                eprintln!("acmex-gen-hooks: --check passed (no diff)");
            }
            return Ok(ExitCode::SUCCESS);
        }
        let recipe = match target {
            EmitTarget::PrePush => "just acmex-gen-hooks",
        ...
        eprintln!(
            "acmex-gen-hooks: --check FAILED - {} is out of sync with the manifest.\n\
             ...
             Regenerate it with:\n    {recipe}\n\
             \n\
             (Or run `cargo run -p acmex-gen-hooks -- --target {tier}` directly.)",
            output_path.display(),
            tier = target.tier(),
        );
...
fn emit_verbose_dump(manifest: &Manifest, manifest_path: &std::path::Path, tier: &str) {
    eprintln!(
        "acmex-gen-hooks: parsed manifest at {} (schema v{}{})",
    ...
    if let Some(cls) = manifest.classification.as_ref() {
        ...
        keys.sort_unstable();
        eprintln!(
            "acmex-gen-hooks: classification keys ({}): {}",
            keys.len(),
            keys.join(", ")
        );
    }
    eprintln!(
        "acmex-gen-hooks: {} gates total, emitting tier `{}`",
        manifest.gate.len(),
        tier
    );
