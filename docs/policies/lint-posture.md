...
ACMEX runs one of the strictest lint configurations of any public Rust
project.  This document is the project's **lint contract**: it
explains *what* we enforce, *why*, and how the trade-offs were chosen
to keep every workspace crate clean under a single shared lint config.

> **Provenance note.** This contract was extracted from a donor
> project (a Windows NTFS file-search tool with a 16-crate workspace).
> The enforced configuration is live in this template's `Cargo.toml` /
> `clippy.toml`; historical anecdotes, probe numbers, and decisions-log
> entries that cite donor crates or donor PRs are inherited donor
> history, kept because they are the *why* behind each setting.
...
- **`cargo-deny`** - license / advisory / source policies (see
  `deny.toml` and [`../../SECURITY.md`](../../SECURITY.md)).
...
ACMEX treats production and test code as two different contexts with
different rules:
...
#[expect(
    ...
    reason = "attribute dispatch - 12 match arms, linear and readable"
)]
fn big_function() { ... }
```
...
ACMEX enables ~100 restriction lints, organised into the following
...
| `multiple_crate_versions` | Heavy ecosystem deps pull transitive version conflicts we cannot resolve; deferred to `cargo-deny` (see `dependency_policy.md` §5). |
| `arbitrary_source_item_ordering` | Too invasive for an established multi-crate layout. |
...
You cannot satisfy both.  ACMEX follows the Rust team's recommendation:
...
- The donor project's lint-consolidation pass recorded **37
  functions** carrying scoped `#[expect(clippy::cognitive_complexity)]`
  - every one an attribute dispatcher, request router, or parser
  orchestrator where extraction would worsen readability.  That is the
  expected shape: a small, named, justified set - not a raised
  ceiling.
...
|---------|-------|--------:|-----|
| (`msrv` / `check-incompatible-msrv-in-tests`) | - | unset | **Intentionally unset.**  The workspace has no `[workspace.package].rust-version` claim while it pins a nightly toolchain (see `rust-toolchain.toml`).  With no MSRV contract, the MSRV-aware lints have nothing to enforce.  See `clippy.toml`'s rationale block. |
...
| `max-include-file-size` | `200_000` | 1_000_000 | Companion to `large_include_file = "deny"`.  200 000 bytes (≈ 195 KiB) allows a deliberate mid-size lookup-table asset (the donor's one legitimate site was a 128 KB case-fold table) while catching accidental megabyte assets. |
...
|---------|--------:|-------------------------|
| `cognitive-complexity-threshold` | 25 | Violations get per-function `#[expect]`. |
| `too-many-lines-threshold` | 100 | Same - only documented pipeline orchestrators should exceed it. |
| `type-complexity-threshold` | 250 | Rarely hit; suppress per-binding when a complex expression type is justified. |
...
ACMEX lints the **same code three times** to defend against
...
the pinned toolchain (see `rust-toolchain.toml` for the current
channel):
...
The template ships fully green.  Every change to `[workspace.lints]`
ships as a green-baseline delta - the workspace is required to pass
`cargo clippy --workspace --all-targets --all-features --locked
--no-deps -- -D warnings` at the time of merge.

---

## 11  Roadmap - Enhancements under evaluation

Every item below was evaluated (in the donor project) against the
then-current Clippy `master` lint index (~814 lints) and the active
toolchain pin.  Each entry carries:
...
- ~~**MSRV pin** - `msrv` + `check-incompatible-msrv-in-tests` in `clippy.toml` (was active on all three lint gates).~~ **Reverted 2026-05-18** - the donor workspace was structurally nightly-only, so the MSRV contract was unverifiable.  Both settings have been removed from `clippy.toml`; see the §12 entry for the audit and §7 above for the current empty-by-design state.
- **Endianness portability** - `host_endian_bytes = "deny"` (workspace clean; the donor's on-disk parser was not using host-endian helpers).
...
on 2026-05-12 - deferred off the main R2 commit for clean bisection
because it carried a large mechanical rename in the donor workspace.
The resulting state is reflected in §4 (restriction-lint buckets), §5
(rustc/rustdoc tables), and §7 (`clippy.toml` settings).  Summary:

- **Bundled-asset cap** - `large_include_file = "deny"` (clippy) + `max-include-file-size = 200_000` (clippy.toml). Verified clean against the donor's only existing site (a 128 KB lookup table).
...
- **Redundant test prefix** - `redundant_test_prefix = "deny"` (restriction). R2-05.  In the donor workspace this drove a `fn test_foo` → `fn foo` mechanical rename across 363 test functions inside `#[cfg(test)]` modules.
...
|------|------|-----|-------------|
| **`field_scoped_visibility_modifiers = "deny"`** | Flag `pub(super)` / `pub(crate)` on **fields** in favour of fully `pub` or private-with-accessors | Visibility scoping clarity | The donor probe measured **44 fields** flagged. The pattern is widely-used idiomatic Rust for crate-internal contracts; adoption requires a design discussion before the surgical refactor (each flagged field → an accessor method). |
...
| **`cargo-hack` feature-matrix in Tier 2** | Run `cargo hack --each-feature check` across feature-bearing crates | We otherwise only test `--all-features`; single-feature breakage is invisible until release | Tier-2 weekly is the right home (`tier-2.yml::hack`) | ✅ **Applied 2026-05-12** - see §12 |
| **`cargo-mutants` in Tier 2 nightly** | Mutation testing scoped to the highest-value library crate (this template: `acmex-core`) | High signal for safety-critical paths | Tier-2 weekly job; `mutants.toml` at repo root supplies shared config (timeout, exclusions, `--locked`) | ✅ **Applied 2026-05-12 (advisory)** - see §12 |
| **`cargo-careful`** | Debug-build hardened-std runner | Complement to existing miri coverage | Tier-2 weekly job (`tier-2.yml::careful`); scope it to your unsafe-density hotspots as the workspace grows | ✅ **Applied 2026-05-12** - see §12 |
...
The entries below are inherited **donor-project history** (crate
names, PR numbers, and probe counts refer to the donor workspace).
They are kept because they document why each currently-enforced
setting exists; append your own entries below them.
...
| 2026-05-11 | `large_include_file = "deny"` (restriction) + `max-include-file-size = 200_000` (clippy.toml) | R2 - cheap structural guard against accidentally bundling a megabyte asset into a release binary. Only legitimate site today is the 128 KB `$UpCase` table in `acmex-text/src/case_fold.rs`; 200 000 bytes (≈ 195 KiB) is generous headroom for that file while still catching accidents. |
...
| 2026-05-12 | `cargo-machete` promoted to a hard gate at pre-push + pr-fast | R3-01.  Fast (~1 s) AST-based unused-dependency detector - complements `cargo-udeps` (Tier 2 weekly; multi-minute, nightly-only) by catching the common case (`[dependencies]` entry with no remaining `use` in the workspace) at PR time instead of a week later.  Wired as `[[gate]] id = "machete"` in `scripts/ci/gates.toml` (`code_changed`, Bucket 1 / `bg`, hard), regenerated `_lint_pre_push.sh` via `just acmex-gen-hooks`, and bundled into `pr-fast.yml`'s existing `security` job step list (display name kept stable to preserve check-run history).  Added to `just install-dev-tools` so the canonical onboarding recipe installs the gate's required binary.  Workspace was already clean against `cargo machete --skip-target-dir` at adoption (verified locally before landing). |
...
| 2026-05-12 | `cargo-hack --each-feature` adopted as Tier 2 weekly job | R3-02.  Audit probed `cargo hack --workspace --each-feature check` and surfaced **three E0432/E0425/E0433 errors** in `acmex-mft` when built with `--no-default-features` - un-gated `use zstd::*` calls hidden by the `default = ["zstd"]` feature flag and the workspace-wide `features = ["zstd"]` pins on every consumer.  Fixed in precursor PR #175 by retiring the `acmex-mft.zstd` feature entirely (promoted to a hard dependency since every consumer enabled it).  With `acmex-mft` clean, this job catches the *next* such regression: any future un-gated `use <crate-behind-feature>` against `acmex-client::async`, `acmex-mcp::streamable-http`, or `acmex-cli::mcp-http-probe`.  Wired as the `hack` job in `tier-2.yml` (between `udeps` and `miri`) running `cargo hack --workspace --exclude acmex-polars --each-feature --keep-going check --all-targets --locked`.  `--exclude acmex-polars` matches the existing polars/chrono cross-compile exclusion in `release-automation-plan.md §8.1` (re-enable when polars upstream ships chrono-compat).  `tier-2-summary.needs` and `notify-failure.needs` lists extended.  Not added to `install-dev-tools` - Tier 2 is the canonical home; the per-feature check rebuilds add up to multi-minute even on warm cache, and PR-time cargo clippy `--all-features` covers the common case. |
...
1. Confirm `rust-toolchain.toml`'s header comment records the bump.
